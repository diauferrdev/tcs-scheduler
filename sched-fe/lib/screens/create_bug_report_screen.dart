        ...
        setState(() {
          _attachments.add(
            AttachmentFile(
              ...
              path: kIsWeb
                  ? ''
                  : image.path, // path only available on native platforms
              bytes: bytes,
              type: mimeType,
            ),
          );
      ...
      if (video != null) {
        ...
        if (bytes.length > 300 * 1024 * 1024) {
          if (mounted) {
            ToastNotification.show(
              context,
              message: 'Video must be less than 300MB',
            );
          }
          return;
        }
        ...
        setState(() {
          _attachments.add(
            AttachmentFile(
              ...
              path: kIsWeb
                  ? ''
                  : video.path, // path only available on native platforms
              bytes: bytes,
              type: mimeType,
            ),
          );
  ...
  Future<void> _pickFile() async {
    if (_attachments.length >= 6) {
      if (mounted) {
        ToastNotification.show(
          context,
          message: 'Maximum 6 attachments allowed',
        );
      }
      return;
    }
          ...
          _attachments.add(
            AttachmentFile(
              ...
              path: kIsWeb
                  ? ''
                  : (file.path ??
                        ''), // Don't access path on web - it throws an error
              bytes: bytes,
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        OutlinedButton.icon(
                          onPressed: _attachments.length < 6
